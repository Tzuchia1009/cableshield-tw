...
export async function openEpisodeModelDialog(dialog,toast){
  try{
    ...
    dialog('接近片段模型與查核排序',
      `<div class="model-hero"><div><span class="status-pill">2026.09.26 · 接近片段模型分析</span><h3>每一個接近片段，<br>都有自己的排序與證據。</h3><p>2023 年訓練、2024 年驗證、2025 年跨年度回溯測試。前處理、森林、自編碼器與百分位參照均以 2023 年 15,398 個合格片段建立。</p></div><div class="model-orbit" aria-label="規則、森林、自編碼器組合為 Top-K"><span>規則</span><span>IF</span><span>AE</span><b>Top<br>K</b></div></div>
      ...
      <div class="model-portfolio"><article><span>A · 明確條件</span><h3>可解釋加權規則</h3><p>距纜、低速、AIS 錨泊狀態、轉向與低速觀測跨度。權重為 0.30、0.25、0.15、0.15、0.15。</p></article><article><span>B · 常態偏離</span><h3>Isolation Forest</h3><p>18 維特徵、300 棵樹。學習 2023 年接近片段的特徵分布。</p></article><article><span>C · 重建誤差</span><h3>自編碼器</h3><p>18 → 32 → 16 → 6 → 16 → 32 → 18；2,488 個參數。三組種子各訓練 80 輪。</p></article></div>
      ...
      <details><summary>獨立延伸研究 CableTrack</summary><p>既有 GRU 航跡預測與物理外推比較仍為獨立研究，未參與片段排序。CableRank 以接近片段進行查核排序；未以人工回饋回訓，亦未完成正式勤務或機關介接驗證。</p></details>
