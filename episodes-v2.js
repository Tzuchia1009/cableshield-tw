...
async function mount(){
  ...
  <p class="episode-note">統計來自受控原始觀測重算；地圖中的 6 小時／0.05 度公開樣本無法用來重建返回次數。距離以 EPSG:3826 計算，時間暫按臺灣時間，海纜為概化示意路由。合格片段已重新訓練模型並重算每日 Top-K；完整結果見「片段模型」。模型分數不從此公開地圖樣本推算。</p>
  ...
  document.getElementById('episodeExport').addEventListener('click',()=>{if(!evidence)return;const url=URL.createObjectURL(new Blob([JSON.stringify(evidence,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='CableShield-片段統計與口徑.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
}
