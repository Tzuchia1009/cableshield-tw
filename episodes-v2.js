...
async function mount(){
  ...
  root.innerHTML=`<div class="episode-heading"><div><span class="eyebrow">接近片段與返回證據</span><h2>同一天反覆接近，每一段都留下來</h2></div><span>每日彙總只供工作量統計</span></div>
  ...
  <div id="returnWindowSensitivity" role="status">正在讀取返回窗口敏感度…</div>
  ...
  <p class="episode-note">統計來自受控原始觀測重算；地圖中的 6 小時／0.05 度公開樣本無法用來重建返回次數。距離以 EPSG:3826 計算，時間暫按臺灣時間，海纜為概化示意路由。合格片段用於模型分析與每日 Top-K 查核排序；完整結果見「片段模型」。模型分數不從此公開地圖樣本推算。</p>
  <button class="button" id="episodeExport">匯出片段統計與口徑</button>`;
  ...
  let evidence=null,windowEvidence=null;
  try{const response=await fetch('./data/episode-v2-summary.json');if(!response.ok)throw new Error('摘要讀取失敗');evidence=await response.json();
    document.getElementById('episodeMetrics').innerHTML=`<div class="episode-metrics"><article><b>${fmt(evidence.episodes)}</b><span>接近片段</span></article><article><b>${fmt(evidence.multi_episode_days)}</b><span>同船同纜同日含多段的彙總</span></article><article><b>${fmt(evidence.repeat_within_24h)}</b><span>24 小時內有觀測支持的返回</span></article><article><b>${fmt(evidence.repeat_not_model_eligible)}</b><span>返回片段未通過模型門檻仍保留</span></article></div><p>資料版本 ${evidence.contract_version}。多段彙總包含缺口分段，不能全部當成返回；${fmt(evidence.uncertain_reappearances)} 段屬中斷後再出現，需補證。${fmt(evidence.model_eligible)} 段通過觀測門檻並已計算片段模型分數；不足以建模的線索仍保留查核價值。</p><details><summary>查看切分參數敏感度</summary><div class="episode-table-wrap"><table><thead><tr><th>進入／離開</th><th>最大缺口</th><th>片段數</th><th>24 小時內返回</th></tr></thead><tbody>${evidence.sensitivity.map(s=>`<tr><td>${s.entry_km}／${s.exit_km} 公里</td><td>${s.gap_hours} 小時</td><td>${fmt(s.episodes)}</td><td>${fmt(s.repeat_within_24h)}</td></tr>`).join('')}</tbody></table></div></details>`;
  }catch(error){document.getElementById('episodeMetrics').textContent='片段統計摘要暫時無法載入；下方僅展示合成切分案例。';}
  try{
    const response=await fetch('./data/return-window-sensitivity.json');if(!response.ok)throw new Error('返回窗口摘要讀取失敗');windowEvidence=await response.json();
    if(windowEvidence.schema_version!=='return-window-sensitivity-1.0.0')throw new Error('返回窗口資料版本不符');
    document.getElementById('returnWindowSensitivity').innerHTML=`<p>${windowEvidence.purpose}</p><details><summary>查看 12／24／48 小時返回窗口敏感度</summary><p>固定 5 公里進入、6 公里離開及 6 小時最大缺口，計算前次片段末次觀測至再次進入的間隔；須有離開後再進入的觀測支持，每個返回片段在各窗口最多計一次。</p><div class="episode-table-wrap"><table><thead><tr><th>返回窗口</th><th>有觀測支持的返回</th><th>未符合模型條件仍保留</th></tr></thead><tbody>${windowEvidence.results.map(s=>`<tr><td>${s.window_hours} 小時</td><td>${fmt(s.confirmed_returns)}</td><td>${fmt(s.not_model_eligible)}</td></tr>`).join('')}</tbody></table></div><p>${windowEvidence.limitations}</p></details><p class="episode-note">CableCheck 用於判斷現有觀測是否足以建立時序與運動特徵，此處檢查的是觀測充分性，不代表事故、責任或實際風險。片段內最大觀測間隔不超過 6 小時，由片段切分規則共同約束，作為一致性檢查，非獨立淘汰條件。</p>`;
  }catch(error){document.getElementById('returnWindowSensitivity').textContent='返回窗口敏感度摘要暫時無法載入。';}
  document.getElementById('episodeExport').addEventListener('click',()=>{if(!evidence)return;const url=URL.createObjectURL(new Blob([JSON.stringify({...evidence,return_window_sensitivity:windowEvidence},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='CableShield-片段統計與口徑.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
}
